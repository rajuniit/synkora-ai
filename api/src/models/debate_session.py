...
class DebateSession(BaseModel, TenantMixin):
    ...
    debate_type = Column(
        String(50), nullable=False, default="structured", comment="structured or freeform"
    )
    ...
    allow_external = Column(
        Boolean, nullable=False, default=False, comment="Whether external agents can join"
    )
    share_token = Column(
        String(64), nullable=True, unique=True, index=True, comment="Token for public share URL"
    )
    participants = Column(
    ...
    synthesizer_agent_id = Column(
        UUID(as_uuid=True), nullable=True, comment="Agent ID for synthesizing verdict"
    )
    ...
    debate_metadata = Column(
        JSON, nullable=True, default=dict, comment="Additional metadata / configuration"
    )
